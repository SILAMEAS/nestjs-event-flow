...
export class AuthServiceService implements OnModuleInit {
  ...
  async register(email: string, password: string, name: string) {
    debugger
    const exist = await this.dbService.db.select().from(users).where(eq(users.email, email)).limit(1);
    if (exist.length > 0) {
  ...
  async getProfile(userId:string){
    const [user]=await this.dbService.db.select({id:users.id,email:users.email,name:users.email,role:users.role}).from(users).where(eq(users.id,userId)).limit(1);
    if(!user){
